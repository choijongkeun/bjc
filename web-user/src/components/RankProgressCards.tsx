...
export function RankProgressCards({
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <Card className="p-6">
        <div className="text-xs uppercase tracking-[0.16em] text-slate-500">Current Rank</div>
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <MetricCard label="LEFT 볼륨" value={metrics ? formatBaseAmount(metrics.left_leg_volume_base, 0) : "-"} />
          <MetricCard label="RIGHT 볼륨" value={metrics ? formatBaseAmount(metrics.right_leg_volume_base, 0) : "-"} />
          <MetricCard label="WEAK 볼륨" value={metrics ? formatBaseAmount(metrics.weak_leg_volume_base, 0) : "-"} />
        </div>
      </Card>

      <Card className="p-6">
        <div className="text-xs uppercase tracking-[0.16em] text-slate-500">Next Rank Progress</div>
