...
test("배치 실행 결과, calc_run summary, reward drill-down, duplicate 재실행을 검증한다", async ({ page, request }) => {
  ...
  await expect(page.getByRole("button", { name: "CONTRIBUTION 실행" })).toBeVisible();
  await expect(page.getByRole("button", { name: "SIDECAR 실행" })).toBeVisible();
  await expect(page.getByText("Rewards 관리")).toBeVisible();
  ...
  await expect(page.getByText("settlement items")).toBeVisible();
});
