...
test("회원가입, 로그인, 로그아웃, 보호 경로 차단을 검증한다", async ({ page }) => {
  ...
  await page.getByLabel("Login ID").fill(`e2e_registered_${fixture.suffix}`);
  await page.getByLabel("Display Name").fill(`Registered ${fixture.suffix}`);
  await page.getByLabel("Password", { exact: true }).fill(fixture.credentials.register_password);
  await page.getByLabel("Password Confirm").fill(fixture.credentials.register_password);
  ...
  await page.getByRole("button", { name: "가입 후 대시보드로 이동" }).click();
...
test("잘못된 비밀번호와 BLOCKED 계정 로그인을 차단한다", async ({ page }) => {
  ...
  await page.getByLabel("Login ID").fill(fixture.credentials.root_user.login_id);
  await page.getByLabel("Password", { exact: true }).fill("WrongPassword!123");
  await page.getByRole("button", { name: "로그인" }).click();
  await expect(page.getByText("로그인 실패")).toBeVisible();

  await page.getByLabel("Login ID").fill(fixture.credentials.blocked_user.login_id);
  await page.getByLabel("Password", { exact: true }).fill(fixture.credentials.blocked_user.password);
  await page.getByRole("button", { name: "로그인" }).click();
  await expect(page.getByText("로그인 실패")).toBeVisible();
});
...
test("로그인 후 auth/me와 로그아웃 후 보호 경로를 다시 확인한다", async ({ page }) => {
  ...
  await expect(page.getByRole("main").getByText(fixture.credentials.root_user.login_id, { exact: true })).toBeVisible();
